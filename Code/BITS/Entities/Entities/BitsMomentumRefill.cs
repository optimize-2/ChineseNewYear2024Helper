...
namespace Celeste.Mod.ChineseNewYear2024Helper.BITS.Entities.Entities {
    public class SpeedPowerup {
        ...
        private static PlayerDeadBody resetSpeedPow(On.Celeste.Player.orig_Die orig, Player self, Vector2 direction, bool evenIfInvincible = false, bool registerDeathInStats = true) {
            ChineseNewYear2024HelperModule.Session.ResetSpeedPowerup();
        ...
        private static void speedPowerBegin(On.Celeste.Player.orig_DashBegin orig, Player self) {
            if (!ChineseNewYear2024HelperModule.Session.HasSpeedPower) { if (ChineseNewYear2024HelperModule.Session.AlwaysBreakDashBlockDash == 1) { ChineseNewYear2024HelperModule.Session.AlwaysBreakDashBlockDash = 2; } orig.Invoke(self); } else {
                self.Play("event:/char/madeline/dash_pink_right", null, 0f);
                if (ChineseNewYear2024HelperModule.Session.CanAddSpeed) {
                    ChineseNewYear2024HelperModule.Session.StoredSpeed += self.Speed;
                    ...
                    self.Speed = value + new Vector2((ChineseNewYear2024HelperModule.Session.Facing == self.Facing ? 1 : -1) * ChineseNewYear2024HelperModule.Session.StoredSpeed.X, ChineseNewYear2024HelperModule.Session.StoredSpeed.Y);
                    ChineseNewYear2024HelperModule.Session.StoredSpeed = Vector2.Zero;
                    Launch = true;
                    self.StateMachine.State = 0;
                }
            }
        ...
        private static void speedPowerEnd(On.Celeste.Player.orig_DashEnd orig, Player self) {
            if (!ChineseNewYear2024HelperModule.Session.HasSpeedPower) { ChineseNewYear2024HelperModule.Session.AlwaysBreakDashBlockDash = 0; orig.Invoke(self); } else {
                if (Store) {
                    ChineseNewYear2024HelperModule.Session.CanAddSpeed = false;
                    Store = false;
                } else if (Launch) {
                    ChineseNewYear2024HelperModule.Session.ResetSpeedPowerup();
                    Launch = false;
    ...
    [CustomEntity("ChineseNewYear2024Helper/BITSMomentumRefill")]
    public class BITSMomentumRefill : Entity {
        ...
        public BITSMomentumRefill(Vector2 position, bool oneUse, float scale)
            : base(position) {
            this.scale = scale;
            ...
            Add(outline = new Image(GFX.Game["objects/ChineseNewYear2024Helper/BitsMomentumRefill/Outline"]));
            outline.CenterOrigin();
            outline.Visible = false;
            Add(sprite = new Image(GFX.Game["objects/ChineseNewYear2024Helper/BitsMomentumRefill/MomentumRefill0"]));
            sprite.Visible = true;
        ...
        protected virtual void OnPlayer(Player player) {
            if (ChineseNewYear2024HelperModule.Session.HasSpeedPower) return;
            ChineseNewYear2024HelperModule.Session.HasSpeedPower = true;
            ChineseNewYear2024HelperModule.Session.CanAddSpeed = false;
            ChineseNewYear2024HelperModule.Session.StoredSpeed = player.Speed;
            ChineseNewYear2024HelperModule.Session.Facing = player.Facing;
